...
def get_summary(
    ...
    if latest_atrium is not None and latest_outside is not None:
        inside = latest_atrium.temperature
        outside = latest_outside.temperature
        difference = abs(inside - outside)

        # Очень большая разница температур
        if difference >= 12:
            health_warning = (
                "🔴 Очень большой перепад температуры между помещением и улицей. "
                "Резкая смена температуры может вызвать стресс для организма. "
                "Рекомендуется выходить постепенно и дать организму адаптироваться."
            )
        ...
        # Большая разница температур
            ...
            health_warning = (
                ...
                "При выходе рекомендуется несколько минут адаптироваться."
            )
        ...
        if outside >= 35:
            health_warning = (
                ...
                "Рекомендуется пить больше воды, избегать длительного пребывания на солнце "
                "и использовать головной убор."
            )

        # Очень холодно на улице
        elif outside <= 0:
            health_warning = (
                "❄️ На улице отрицательная температура. "
                "Перед выходом рекомендуется надеть теплую одежду "
                "и избегать резкой смены температуры."
            )
